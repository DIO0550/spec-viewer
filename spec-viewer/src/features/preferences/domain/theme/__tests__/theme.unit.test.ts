...
import { ThemeAppearance, ThemeMode } from "@/features/preferences/domain/theme";

test.each(["system", "light", "dark"] as const)(
  "ThemeModeは%sを有効なmodeとして判定する",
  (themeMode) => {
...
test.each([null, undefined, "", "blue", 1, false] as const)(
  "ThemeModeは無効値%jをsystemへfallbackする",
  (value) => {
...
test.each(["light", "dark"] as const)(
  "ThemeAppearanceは%sを有効なappearanceとして判定する",
  (themeAppearance) => {
...
test.each([
  ...
  "ThemeMode.toAppearanceはsystem modeで現在のsystem appearanceを返す",
...
test.each([
  ...
  "ThemeMode.toAppearanceは明示modeでsystem appearanceを無視する",
  ({ mode, systemAppearance, expected }) => {
