...
test("ReviewSession.fromReviewRunは非archived runをactive collection向けにnarrowする", () => {
  const entity = ReviewSession.fromReviewRun(activeRun);

  expect(ReviewSession.isNonArchived(entity)).toBe(true);
  expect(ReviewSession.isArchived(entity)).toBe(false);
});

test("ReviewSession.fromReviewRunはarchived runをarchived variantへnarrowする", () => {
  const entity = ReviewSession.fromReviewRun(
    createReviewRun({
      id: "run-archived",
      status: "archived",
...
test("ReviewSession.fromReviewRunはarchivedAtのないarchived runを拒否する", () => {
  ...
  expect(() => ReviewSession.fromReviewRun(invalidRun)).toThrow(
    "Archived review run must have archivedAt",
  );
});

test("ReviewSession.fromReviewRunはarchivedAtのある非archived runを拒否する", () => {
  const invalidRun = createReviewRun({
