...
test("resolve が false を返すと edit popover に error を表示する", async () => {
  const comment = createComment();
  const baseProps = createLayerProps({ comments: [comment] });
  const props = {
    ...baseProps,
  ...
  act(() => editButton?.click());
  const resolveButton = result.container.querySelector<HTMLButtonElement>(
    ".add-comment-popover__status-actions .button--secondary",
  );
  ...
  const error = result.container.querySelector(".add-comment-popover__error");
  expect(error).not.toBeNull();
  expect(error?.textContent).toContain("コメントを解決できませんでした");
  result.unmount();
});
