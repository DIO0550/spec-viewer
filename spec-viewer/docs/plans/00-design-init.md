...
Treat the design model as the preferred UI/product direction while keeping filesystem logic configurable enough to support both:
...
1. Build a workspace detector that can identify `.plugin-workspace/.specs/` and `.spec-skill/features/`.
...
4. Keep `.spec-skill` support as a compatibility mode.
