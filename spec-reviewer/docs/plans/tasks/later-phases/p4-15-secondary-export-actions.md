...
- [ ] Remove always-visible download/export buttons from the main comment sidebar body.
- [ ] Keep raw comment export available from a secondary menu such as `その他` or `エクスポート`.
- [ ] Keep prompt copy/export available from the same secondary menu or a review-specific overflow menu.
- [ ] Keep `レビュー作成` visible as the primary action for handing comments to an AI workflow.
- [ ] Use Japanese labels that distinguish raw export from user-review creation.
- [ ] Keep export success/error feedback visible only after the user triggers an export.
- [ ] Preserve existing export and prompt-generation backend behavior.
- [ ] Update component tests that currently expect visible download/export buttons.
- [ ] Add tests that export actions are reachable from the secondary menu and no longer dominate the empty/comment list state.
